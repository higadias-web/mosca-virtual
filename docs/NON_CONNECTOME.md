...
| A2 | Atuadores de torque nas 42 juntas ativas; sem adesão; rigidez/amortecimento passivos de `make_locomotion_fly` (0,05 / 0,06), **sem limites de amplitude** | idem | **DEFEITO (Sessão 2):** essa rigidez é residual, porque no corpo original quem segura as juntas são os atuadores de posição (kp 45). Com torque puro as juntas giram várias voltas. Correção proposta em FASE3_PLANO §7.3 |
| A2' | (Sessão 3; implementado em `make_neuromuscular_fly(passive="wang2025")`, **validação pendente**) Correção do A2: rigidez passiva por junta e perna medida em *Drosophila* (**PREPRINT, sem revisão por pares**: Wang et al. 2025, bioRxiv 10.1101/2025.04.29.651225; unidade impressa "mN/°", lida como mN·m/° (**hipótese nossa**, checada pelo multiplicador de sustentação, FASE3_PLANO §7.6.3; **a checagem pré-registrada FALHOU**: m* = 2,5 contra [10, 160], discrepância de 16× com o ×40 do artigo; as outras leituras ficam a ≥ 62×; mantida como valor principal por decisão do usuário, opção (i) de §7.7; sensibilidade a k/16 no loop, §7.8.3): ThC pitch, ThC roll, CTr, FTi; 0,11–3,2 µN·mm/rad); ThC yaw, TrF roll e TiTa = mediana da perna (**sem fonte**); repouso na pose neutra do NeuroMechFly (**sem fonte**); amortecimento c = max(k·8 ms, 2√(kI)) (**critério fixado antes, sem fonte**); limites = faixa da marcha real ± 30 % da amplitude (**sem fonte**; NeuroMechFly não tem limites, flybody usa outra referência, FlyMimic só L1 e sem origem); limite macio do MuJoCo: padrão (0,02 s), "std2dt" (0,0002 s) e "direct_dt" (solref −1e8/−2e4) testados; **os três reprovados no teste (c)** (§7.9, §7.11), e a 3a foi encerrada | `apparatus.py` | tabela e critério em FASE3_PLANO §7.4; sem ajuste depois de ver resultados |
...
| B-vin5 | **Taxa dos ORNs COM FONTE** (condição de referência): aumento sobre a taxa espontânea de ab1A/ab1B para vinagre de maçã (Faucher, Hilker & de Bruyne 2013, *PLoS ONE* 8:e56361, Fig. 2C, leitura visual ±3 spikes/s): 0,1 % → 7/0 Hz (ponto sem separação por sexo); 0,5 % → 16/11; **5 % → 42/22**; 50 % → 88/42 (fêmeas). **Faucher mede o AUMENTO sobre a taxa espontânea** (Métodos: "Pre-stimulus activity was then subtracted…"), **e o modelo não tem atividade de fundo**: a taxa espontânea dos ORNs (não informada na fonte) fica de fora. **Correspondência dos neurônios, com fonte lida:** ab1A = Or42b e ab1B = Or92a (Faucher 2013, Fig. 2B: "Or42b-DTi (A neuron deleted)", "Or92a-DTi (B neuron deleted)"); Or42b → DM1 e Or92a → VA2 (Semmelhack & Wang 2009, *Nature* 459:218: "the Or42b neurons, which innervate the DM1 glomerulus"; "the Or92a neurons, which innervate the VA2 glomerulus"). **Sem fonte:** a concentração perto da fruta, a curva das fêmeas e a leitura da figura | `experiments/phase3b_rates.py` | substitui os 100 Hz sem fonte como referência; 20/50/100 Hz ficam como varredura |
