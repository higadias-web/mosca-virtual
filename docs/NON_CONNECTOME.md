...
| A2' | (Sessão 3; implementado em `make_neuromuscular_fly(passive="wang2025")`, **validação pendente**) Correção do A2: rigidez passiva por junta e perna medida em *Drosophila* (Wang et al. 2025, bioRxiv 10.1101/2025.04.29.651225, preprint: ThC pitch, ThC roll, CTr, FTi; 0,11–3,2 µN·mm/rad); ThC yaw, TrF roll e TiTa = mediana da perna (**sem fonte**); repouso na pose neutra do NeuroMechFly (**sem fonte**); amortecimento c = max(k·8 ms, 2√(kI)) (**critério fixado antes, sem fonte**); limites = faixa da marcha real ± 30 % da amplitude (**sem fonte**) | `apparatus.py` | tabela e critério em FASE3_PLANO §7.4; sem ajuste depois de ver resultados |
