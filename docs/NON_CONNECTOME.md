...
| H5-bg | (Sessão 2) **Atividade de fundo fraca: Poisson de 5 Hz** em cada MN de perna (391) e em cada pré-motor direto (5.694 intrínsecos do VNC com ≥ 5 sinapses em MNs de perna), com o refratário normal mantido (diferente dos alvos estimulados do Shiu, que têm refratário 0). No LIF do Shiu cada evento de Poisson (w_syn·f_poi = 68,75 mV) dispara o neurônio, então a taxa espontânea resultante ≈ 5 Hz. Critério, fixado antes de rodar: fundo **fraco** = bem abaixo da taxa de repouso do único MN de perna que dispara parado (MN lento da tíbia, ~30 Hz; os rápidos e intermediários ficam silenciosos; Azevedo et al. 2020, eLife 9:e56754). Para os pré-motores não há medida de repouso; o mesmo nível é uma escolha. Controle: só fundo, sem aferência, mesmas sementes | `experiments/phase3a_s2.py` | ritmo só conta se superar o controle |
